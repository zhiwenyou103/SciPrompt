...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, 
                       default="/home/zhiweny2/chatbotai/jerome/KAPT/S2ORC/",
                       help='Data directory')
    parser.add_argument('--verbalizer_path', type=str,
                       default='/home/zhiweny2/chatbotai/jerome/KAPT/filtered_knowledge_words/S2ORC_knowledgable_verbalizer.txt',
                       help='Path to verbalizer file')
    parser.add_argument('--semantic_score_path', type=str,
                       default='/home/zhiweny2/chatbotai/jerome/KAPT/filtered_knowledge_words/S2ORC_knowledgable_verbalizer_semantic_search_scores.txt',
...
def run_zero_shot(args, prompt_model, dataset, mytemplate, myverbalizer, tokenizer, WrapperClass, device, class_labels):
    ...
    if args.calibration:
        support_dataloader = create_prompt_dataloader(
            dataset["train"], mytemplate, tokenizer, WrapperClass, args
        )
    ...
    test_dataloader = create_prompt_dataloader(
        dataset["test"], mytemplate, tokenizer, WrapperClass, args
    )
...
def run_few_shot_training(args, prompt_model, dataset, mytemplate, myverbalizer, tokenizer, WrapperClass, device, class_labels):
    ...
    train_dataloader = create_prompt_dataloader(
        dataset["train"], mytemplate, tokenizer, WrapperClass, args, shuffle=True
    )
    validation_dataloader = create_prompt_dataloader(
        dataset["validation"], mytemplate, tokenizer, WrapperClass, args
    )
    test_dataloader = create_prompt_dataloader(
        dataset["test"], mytemplate, tokenizer, WrapperClass, args
    )
...
def main():
    ...
    dataset = {
        ...
        'test': get_examples(
            args.data_dir,
            "300_random_sample_test_dataset",
            "300_random_sample_test_labels",
            tokenizer,
            label_dict
        )
    }
