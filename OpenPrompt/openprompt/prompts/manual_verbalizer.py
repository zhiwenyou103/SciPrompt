...
class WeightedVerbalizer(Verbalizer):
    r"""
    ...
    def generate_parameters(self) -> List:
        ...
        for scores, ids in zip(all_scores, all_ids):
            assert len(scores) == len(ids), "The number of label words and scores should be equal"
        ...
        self.label_words_scores = nn.Parameter(torch.clamp(words_scores_tensor.max(dim=-1)[0], min=0), requires_grad=True)
    ...
    def aggregate(self, label_words_logits: torch.Tensor) -> torch.Tensor:
        ...
        if not self.training:
            label_words_scores_weights = F.softmax(self.pred_temp*self.label_words_scores-10000*(1-self.label_words_mask), dim=-1)
        else:
            label_words_scores_weights = F.softmax(self.label_words_scores-10000*(1-self.label_words_mask), dim=-1)
        label_words_logits = (label_words_logits * self.label_words_mask * label_words_scores_weights).sum(-1)
        return label_words_logits
