...
class SoftVerbalizer(Verbalizer):
    ...
    def __init__(self,
        ...
        head_name = [n for n,c in model.named_children()][-1] # cls
    ...
    def generate_parameters(self) -> List:
        ...
        for scores_per_class, ids_per_label in zip(all_scores, all_ids):
            assert len(scores_per_class) == len(ids_per_label), "The number of label words and scores should be equal"
        ...
        words_scores = [[score*len(ids) + [0]*(max_len-len(ids)) for score, ids in zip(scores_per_class, ids_per_label)]
                             + [[0]*max_len]*(max_num_label_words-len(ids_per_label))
                             for scores_per_class, ids_per_label in zip(all_scores, all_ids)]
        ...
        words_scores_tensor = torch.tensor(words_scores)
        
        self.label_words_ids = nn.Parameter(words_ids_tensor, requires_grad=False)
        self.new_label_words_ids = nn.Parameter(torch.clamp(words_ids_tensor.sum(dim=-2), min=0), requires_grad=False)
        self.words_ids_mask = nn.Parameter(words_ids_mask, requires_grad=False) # 3-d mask shape: (num_classes, max_num_label_words, max_len)
        self.new_words_ids_mask = nn.Parameter(torch.clamp(words_ids_mask.sum(dim=-2), min=0), requires_grad=False)
        self.label_words_mask = nn.Parameter(torch.clamp(words_ids_mask.sum(dim=-1), max=1), requires_grad=False)

        self.label_words_scores = nn.Parameter(torch.clamp(words_scores_tensor.mean(dim=-1), min=0), requires_grad=True)
        self.label_words_weights = nn.Parameter(torch.zeros(self.num_classes, max_num_label_words), requires_grad=True)

        if not self.training:
            label_words_scores_weights = F.softmax(self.pred_temp*self.label_words_scores, dim=-1) 
        else:
            label_words_scores_weights = F.softmax(self.label_words_scores, dim=-1) 
        
        # incorporate label words weights:
        if not self.training:
        ...
        init_data = self.original_head_last_layer[self.label_words_ids,:]
        init_data = (init_data * self.words_ids_mask.unsqueeze(-1)).sum(dim=-2)/(self.words_ids_mask.unsqueeze(-1).sum(dim=-2)+1e-15)
        init_data = init_data * self.label_words_scores.to(self.original_head_last_layer.dtype).unsqueeze(-1) * self.label_words_mask.to(self.original_head_last_layer.dtype).unsqueeze(-1)
    ...
    def process_hiddens(self, hiddens: torch.Tensor, **kwargs):
        r"""A whole framework to process the original logits over the vocabulary
        """
    ...
    def process_outputs(self, outputs: torch.Tensor, batch: Union[Dict, InputFeatures], **kwargs):
        ...
        init_data = (outputs[:, self.words_ids_mask] * self.words_ids_mask.unsqueeze(0)).sum(dim=-1)/(self.words_ids_mask.unsqueeze(0).sum(dim=-1)+1e-15)
        init_data = init_data.sum(dim=-1)
        new_outputs = outputs * init_data.sum(-1).unsqueeze(-1) * 0.1 
        ...
        for i_label, words_ids_per_label in enumerate(cur_label_words_ids):
            ...
            for j_word, word_ids in enumerate(words_ids_per_label):
                ...
                if len((set(word_ids).difference(set([0]))).intersection(rm_calibrate_ids)) == 0:
                    ...
                    new_label_words_scores[-1].append(all_scores[i_label][j_word])

        self.set_tagger = True
        self.new_label_words_scores = new_label_words_scores
